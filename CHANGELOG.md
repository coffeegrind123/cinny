...
- `abe435d` Fixed silent version drift in the release pipeline that broke the Windows updater and made Android re-download every launch — `scripts/bump-version.mjs` was reading the version from `git describe --tags --abbrev=0`, but the new tag is created by `gh release create --draft` which defers the actual git tag to publish-time, so every platform job stamped the PREVIOUS version into installers / signatures / `tauri.properties`. Now the version is plumbed from `create-release` to every platform job via a `PRINNY_VERSION` env var; `bump-version.mjs` prefers it over `git describe` and logs which source it used.
